...
var (
	runeToUint64LookupTable = map[rune]uint64{
		DigitZero  : 0,
		DigitOne   : 1,
		DigitTwo   : 2,
		DigitThree : 3,
...
// Uint64 tries to convert the numeric value that a rune represents to an
// uint64.
//
// Note, of course, not all runes represent numeric values. In those cases
// Uint64 returns an error of type NotNumericComplainer.
//
// Also, if the rune is numeric but cannot be represented as an uint64,
// then Uint64 returns an error of type NotInRangeComplainer.
func Uint64(r rune) (uint64, error) {
	if x, ok := runeToUint64LookupTable[r]; !ok {
		if !IsNumeric(r) {
